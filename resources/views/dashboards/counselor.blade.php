...
<div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
      ...
      <table class="min-w-full text-sm">
        ...
        <tbody id="counselorCaseFeed" class="divide-y divide-slate-100">
          ...
          <tr>
            ...
            <td class="px-6 py-4">
              <span class="bg-rose-100 text-rose-700 text-xs font-semibold px-2 py-1 rounded-full">For Case Conference</span>
            </td>
          </tr>
          <tr>
            ...
            <td class="px-6 py-4">
              <span class="bg-amber-100 text-amber-700 text-xs font-semibold px-2 py-1 rounded-full">Needs Parent Call</span>
            </td>
          </tr>
          <tr>
            ...
            <td class="px-6 py-4">
              <span class="bg-rose-100 text-rose-700 text-xs font-semibold px-2 py-1 rounded-full">Restorative Session Pending</span>
            </td>
          </tr>
        </tbody>
      </table>
  ...
  <section class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
    ...
    <div id="resultCleared" class="hidden mt-5 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
      <p class="text-sm font-semibold text-emerald-700"><i class="fa-solid fa-circle-check mr-1"></i> Cleared: Generate PDF</p>
      <p class="text-xs text-emerald-700/80 mt-1" id="clearedMsg">Learner has no unresolved major offense.</p>
      <button class="mt-3 bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-lg text-sm font-semibold">
    ...
    <div id="resultBlocked" class="hidden mt-5 rounded-xl border border-rose-200 bg-rose-50 p-4">
      <p class="text-sm font-semibold text-rose-700"><i class="fa-solid fa-circle-xmark mr-1"></i> Blocked: Unresolved Major Offense</p>
      <p class="text-xs text-rose-700/80 mt-1" id="blockedMsg">Learner has a pending case with no recorded restorative completion.</p>
    </div>

    {{-- Default (pre-search) state --}}
    <div id="resultDefault" class="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-400 italic">
...
<script>
  ...
  const blockedLRNs = {
    '112233445566': { name: 'Mark Angelo Pineda', offense: 'Bullying' },
    '109944887722': { name: 'Ralph Christian Ramos', offense: 'Cheating' },
  };
